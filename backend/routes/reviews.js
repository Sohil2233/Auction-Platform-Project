...
router.post("/", auth, async (req, res) => {
  try {
    const { reviewedUserId, listingId, transactionId, rating, title, content, isAnonymous } = req.body;
    ...
    const transaction = await Transaction.findOne({
      ...
      listing: listingId,
      $or: [
        { buyer: reviewerId, seller: reviewedUserId },
        { buyer: reviewedUserId, seller: reviewerId }
      ],
      status: 'completed'
    });

    if (!transaction) {
      return res.status(400).json({ message: "Can only review users after completed transaction" });
    }
